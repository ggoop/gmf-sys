...
export default function (nav, options = {}) {
  ...
  if (typeof scrollBox === 'string') {
    ...
    if (!scrollBox) {
      if (process.env.NODE_ENV === 'development') {
        console.error('[MD] sticky:scroll-box element doesn\'t exist')
      }
      return
    }
  }
  ...
  const getFillElem = function (el) {
    ...
    if (next.classList.contains('md-sticky-fill')) {
      return next
    }
  ...
  const scrollHandler = function () {
    ...
    if (distance > navOffsetY) {
      ...
      nav.classList.add('md-sticky-fixed')
    } else {
      nav.classList.remove('md-sticky-fixed')
    }
  }
  ...
  if (checkStickySupport && (gtIOS6() || isSupportSticky())) {
    nav.style.top = offset + 'px'
    // 大于等于iOS6版本使用sticky
    nav.classList.add('md-sticky')
  } else {
    if (nav.classList.contains('md-sticky-fixed')) {
      const top = getTop()
