...
<script>
  ...
  export default {
    ...
    computed: {
      currentData() {
        if (this.pieOptions) {
          return this.mdData.slice().map(item => {
            item.a = '1'
            return item
          })
        }
      ...
      currentXFieldOptions(val) {
        ...
        if (!this.xFieldOptions) {
          if (/\d{4}-\d{2}-\d{2}/.test(this.mdData[0][this.xField])) {
    ...
    methods: {
      ...
      getFields() {
        ...
        if (this.mdData && this.mdData.length) {
          const keys = Object.keys(this.mdData[0])
          if (keys.length >= 2) {
            let indexes = [0, 1]
            const type1 = typeof this.mdData[0][keys[0]]
            const type2 = typeof this.mdData[0][keys[1]]
      ...
      render() {
        ...
        const chart = new F2.Chart({
          ...
          height: this.height ? this.height : (windowWidth > windowHeight ? (windowHeight - 54) : windowWidth *
            0.707),
        ...
        if (this.pieOptions) {
          ...
          chart.axis(false)
          chart.interval()
            .position(this.buildPosition()) //'a*percent'
            .color(this.pieOptions.seriesField, (this.pieOptions.colors && this.pieOptions.colors.length) ? this.pieOptions
              .colors : '')
            .adjust('stack')
            .style({
              lineWidth: 1,
