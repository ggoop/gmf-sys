  ...
  <div :class="b([type])">
    <div ref="wrap" :class="[
      ...
      <div :class="b('nav', [type])" ref="nav">
        ...
        <div v-for="(tab, index) in tabs" ref="tabs" class="md-x-tab" :class="{
            'md-x-tab--active': index === curActive,
            'md-x-tab--disabled': tab.disabled
          }" @click="onClick(index)" :key="index">
          <span class="md-ellipsis" ref="title">{{isObj(tab.title)?tab.title.name:tab.title }}</span>
        </div>
      </div>
    </div>
...
<script>
  ...
  import {
    raf
  } from 'gmf/core/utils/MdRaf';
  import {
    on,
    off
  } from 'gmf/core/utils/MdInteractionEvents';
