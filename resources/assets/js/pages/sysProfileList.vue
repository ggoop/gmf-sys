...
<script>
  export default {
    ...
    methods: {
      create(){
        this.$router.push({ name: 'module', params: { module: 'sys.profile.edit' }});
      },
      edit(item){
        this.$router.push({ name: 'id', params: { module: 'sys.profile.edit',id:item.id }});
      },
      remove(){
        ...
        this.$http.delete('sys/profiles/'+ids).then(response => {
          this.load();
          this.loading--;
