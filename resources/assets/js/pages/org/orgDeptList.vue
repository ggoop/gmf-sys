<template>
  <md-part>
    ...
    <md-part-body>
      <md-query @select="select" @dblclick="edit" ref="list" md-query-id="gmf.org.dept.list"></md-query>
...
<script>
  export default {
    ...
    methods: {
      create(){
        this.$router.push({ name: 'module', params: { module: 'org.dept.edit' }});
      },
      edit(item){
        this.$router.push({ name: 'id', params: { module: 'org.dept.edit',id:item.id }});
      },
      remove(){
        ...
        this.$http.delete('org/depts/'+ids).then(response => {
          this.load();
          this.loading--;
