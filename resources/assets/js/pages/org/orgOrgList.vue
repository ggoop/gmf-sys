<template>
  <md-part>
    ...
    <md-part-body>
      <md-query @select="select" @dblclick="edit" ref="list" md-query-id="gmf.org.org.list"></md-query>
...
<script>
  export default {
    ...
    methods: {
      create(){
        this.$router.push({ name: 'module', params: { module: 'org.org.edit' }});
      },
      edit(item){
        this.$router.push({ name: 'id', params: { module: 'org.org.edit',id:item.id }});
      },
      remove(){
        ...
        this.$http.delete('org/orgs/'+ids).then(response => {
          this.load();
          this.loading--;
